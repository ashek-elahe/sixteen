...
void showAnimatedDialog(Widget dialog, {bool isFlip = false, bool dismissible = true}) {
  showGeneralDialog(
    context: Get.context!,
    ...
    transitionBuilder: (context, a1, a2, widget) {
      if(isFlip) {
        return Rotation3DTransition(
          ...
          turns: Tween<double>(begin: math.pi, end: 2.0 * math.pi).animate(CurvedAnimation(
            parent: a1, curve: const Interval(0.0, 1.0, curve: Curves.linear),
          )),
          child: FadeTransition(
            opacity: Tween<double>(begin: 0.0, end: 1.0).animate(CurvedAnimation(
              parent: a1, curve: const Interval(0.5, 1.0, curve: Curves.elasticOut),
            )),
            child: widget,
          ),
        );
